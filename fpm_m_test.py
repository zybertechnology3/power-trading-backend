...
def _public_holidays_for_year(year: int) -> tuple[dict, ...]:
    ...
    return tuple(_normalize_public_holiday(record) for record in records)
...
def _holiday_dates_for_range(start_date: date, end_date: date) -> set[date]:
    ...
    for year in range(start_date.year - 1, end_date.year + 2):
        actual_dates.update(
            holiday["date"]
            for holiday in _public_holidays_for_year(year)
            if not holiday["types"] or "Public" in holiday["types"]
        )
...
def extract_hourly_table(driver, dataset: str, timeout: int = 20) -> dict:
    print(f"📊 [6/7] Reading {dataset} table")
    wait_for_page_settle(driver, timeout=timeout, extra_delay=1.5)
...
def enrich_hourly_records(
    ...
    for monthstart in planned_monthstarts:
        ...
        target_month_prices = monthly_prices.get(target_month_text)
        if target_month_prices is None:
            continue
    ...
    return enriched_records, {
        "expected_monthstarts": [monthstart.isoformat() for monthstart in planned_monthstarts],
        "missing_monthstarts": missing_monthstarts,
        "returned_monthstarts": sorted(monthly_prices.keys()),
    }
...
def filter_existing_records(collection, records: list[dict]) -> tuple[list[dict], int]:
    ...
    if len(unique_records) != len(records):
        print(
            f"🧹 [db] Deduplicated {len(records) - len(unique_records)} rows before write "
...
def close_mongo_connection(client: MongoClient) -> None:
    client.close()
    print("🗄️ [db] MongoDB connection closed")
...
def store_results(
    ...
    print(
        f"💾 [db] Stored batch: "
        f"unconstrained={summary['unconstrained']['imported']} imported, "
        f"constrained={summary['constrained']['imported']} imported"
    )
...
def verify_storage_coverage(
    ...
    def summarize(collection, dataset: str, records: list[dict]) -> None:
        if not records:
            print(f"  ℹ️ {dataset}: no records prepared")
            return
        ...
        print(
            f"  ✓ {dataset}: expected {expected_start}..{expected_end}, "
            f"stored={len(stored_dates)}, missing={len(missing_dates)}"
        )
...
def scrape_area_results_for_search_date(
    ...
    second_dataset = "constrained" if start_with_unconstrained else "unconstrained"
    second_toggle_state = not first_toggle_state

    click_order = [
        (first_dataset, first_toggle_state),
        (second_dataset, second_toggle_state),
    ]
    ...
    for step_index, (dataset_name, toggle_state) in enumerate(click_order):
        ...
        records[dataset_name], validation[dataset_name]["monthstart_validation"] = enrich_hourly_records(
            dataset_name,
...
def run(
    ...
    print(
        f"🚀 [run] start={start_date.isoformat()} end={end_date.isoformat()} "
        f"chunks={len(chunks)} timeout={timeout}s headless={headless}"
    )
    print(f"🌴 [run] Holiday-aware TOU active for {len(holiday_dates)} dates")
    ...
    try:
        ...
        missing_monthstarts_by_dataset = {"unconstrained": [], "constrained": []}
        ...
        for chunk_result, storage_future in storage_jobs:
            ...
            for dataset_name in ("unconstrained", "constrained"):
                ...
                missing_monthstarts_by_dataset[dataset_name].extend(
                    dataset_validation.get("monthstart_validation", {}).get(
                        "missing_monthstarts", []
                    )
                )
        ...
        print(
            f"⚠️ [7/7] missing monthstarts: "
            f"unconstrained={sorted(set(missing_monthstarts_by_dataset['unconstrained']))}, "
            f"constrained={sorted(set(missing_monthstarts_by_dataset['constrained']))}"
        )
        run_succeeded = True
        return result
    finally:
